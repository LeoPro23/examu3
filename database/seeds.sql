...
-- Ubicaciones adicionales (Check existence manually as there is no unique constraint)
INSERT INTO ubicaciones (edificio, aula_oficina, descripcion)
SELECT 'Edificio A', 'Aula 102', 'Aula teórica'
WHERE NOT EXISTS (SELECT 1 FROM ubicaciones WHERE edificio='Edificio A' AND aula_oficina='Aula 102');

INSERT INTO ubicaciones (edificio, aula_oficina, descripcion)
SELECT 'Edificio B', 'Biblioteca', 'Sala de cómputo biblioteca'
WHERE NOT EXISTS (SELECT 1 FROM ubicaciones WHERE edificio='Edificio B' AND aula_oficina='Biblioteca');

INSERT INTO ubicaciones (edificio, aula_oficina, descripcion)
SELECT 'Edificio C', 'Auditorio', 'Auditorio principal'
WHERE NOT EXISTS (SELECT 1 FROM ubicaciones WHERE edificio='Edificio C' AND aula_oficina='Auditorio');

INSERT INTO ubicaciones (edificio, aula_oficina, descripcion)
SELECT 'Edificio C', 'Almacén TI', 'Almacén principal de equipos'
WHERE NOT EXISTS (SELECT 1 FROM ubicaciones WHERE edificio='Edificio C' AND aula_oficina='Almacén TI');
...
-- Mantenimientos
INSERT INTO mantenimientos (equipo_id, tipo, fecha_programada, fecha_realizada, tecnico_id, proveedor_id, descripcion, problema_reportado, solucion_aplicada, costo, estado, prioridad)
SELECT (SELECT id FROM equipos WHERE codigo_inventario='PC-003'), 'correctivo', '2024-11-01', '2024-11-02', (SELECT id FROM usuarios WHERE username='tecnico1'), NULL, 'Falla de disco duro', 'PC no inicia', 'Cambio de HDD por SSD', 80.00, 'completado', 'alta'
WHERE NOT EXISTS (SELECT 1 FROM mantenimientos WHERE equipo_id=(SELECT id FROM equipos WHERE codigo_inventario='PC-003') AND fecha_programada='2024-11-01');

INSERT INTO mantenimientos (equipo_id, tipo, fecha_programada, fecha_realizada, tecnico_id, proveedor_id, descripcion, problema_reportado, solucion_aplicada, costo, estado, prioridad)
SELECT (SELECT id FROM equipos WHERE codigo_inventario='IMP-001'), 'preventivo', '2024-12-15', NULL, NULL, (SELECT id FROM proveedores WHERE ruc='20555666777'), 'Limpieza general y cambio de toner', NULL, NULL, 0.00, 'programado', 'media'
WHERE NOT EXISTS (SELECT 1 FROM mantenimientos WHERE equipo_id=(SELECT id FROM equipos WHERE codigo_inventario='IMP-001') AND fecha_programada='2024-12-15');

INSERT INTO mantenimientos (equipo_id, tipo, fecha_programada, fecha_realizada, tecnico_id, proveedor_id, descripcion, problema_reportado, solucion_aplicada, costo, estado, prioridad)
SELECT (SELECT id FROM equipos WHERE codigo_inventario='PRJ-001'), 'correctivo', '2024-10-10', '2024-10-12', (SELECT id FROM usuarios WHERE username='tecnico2'), NULL, 'Cambio de lámpara', 'Imagen oscura', 'Reemplazo de lámpara', 150.00, 'completado', 'media'
WHERE NOT EXISTS (SELECT 1 FROM mantenimientos WHERE equipo_id=(SELECT id FROM equipos WHERE codigo_inventario='PRJ-001') AND fecha_programada='2024-10-10');

-- Movimientos
INSERT INTO movimientos_equipos (equipo_id, ubicacion_origen_id, ubicacion_destino_id, usuario_responsable_id, motivo, observaciones)
SELECT (SELECT id FROM equipos WHERE codigo_inventario='PC-003'), (SELECT id FROM ubicaciones WHERE aula_oficina='Lab 101'), (SELECT id FROM ubicaciones WHERE aula_oficina='Oficina TI'), (SELECT id FROM usuarios WHERE username='tecnico1'), 'Reparación', 'Se lleva a taller para cambio de disco'
WHERE NOT EXISTS (SELECT 1 FROM movimientos_equipos WHERE equipo_id=(SELECT id FROM equipos WHERE codigo_inventario='PC-003') AND motivo='Reparación');

INSERT INTO movimientos_equipos (equipo_id, ubicacion_origen_id, ubicacion_destino_id, usuario_responsable_id, motivo, observaciones)
SELECT (SELECT id FROM equipos WHERE codigo_inventario='PC-003'), (SELECT id FROM ubicaciones WHERE aula_oficina='Oficina TI'), (SELECT id FROM ubicaciones WHERE aula_oficina='Lab 101'), (SELECT id FROM usuarios WHERE username='tecnico1'), 'Retorno', 'Equipo reparado y devuelto'
WHERE NOT EXISTS (SELECT 1 FROM movimientos_equipos WHERE equipo_id=(SELECT id FROM equipos WHERE codigo_inventario='PC-003') AND motivo='Retorno');
